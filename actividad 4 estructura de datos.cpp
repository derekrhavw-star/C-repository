...
class Alumnos_ingenieria{
    protected: 
    int programacion; 

    public:
        Alumnos_ingenieria(){}; //constructor por defecto. Es importante para poder crear variables de la clase parametrizadas

    void setProgramacion(int p){ //al ser protected la variable no sé si sea obligatorio el uso de getters y setters, al ser un punto medio de private y public, deberia preguntarle al profe (!!)
        programacion = p;
    }
    ...
    virtual int Promedio_programador(){
        int promedio = programacion;
        cout<<"Ingrese la calificacion de programacion: "<<endl;
        cin >> programacion;
        cout<<"El promedio del alumno en programacion es: "<<promedio<<endl;
        return promedio;
    }

};
 
class Alumnos : public Alumnos_ingenieria{ 
    private: 
    ...
    int obtener_promedio(Alumnos alumno1, Alumnos alumno2){ 
     ...
     int Promedio_programador() override{
        
     cout<<"Ingrese la calificacion de programacion de su alumno: "<<endl;
     cin>>programacion;
        while((programacion < 60 || programacion > 100) || (cin.fail())){ 
                cout<<"Ingrese una calificacion valida (60-100): "<<endl; 
                cin.clear(); 
                cin.ignore(1000, '\n'); 
                cin >> programacion;
        }   
     cout<<"Ahora ingrese las calificaciones de matematicas, fisica y quimica respectivamente, estas deben tener un rango de 60 a 100:"<<endl;
     cin>> mate;
        while((mate < 60 || mate > 100) || (cin.fail())){ 
            cout<<"Ingrese una calificacion valida (60-100): "<<endl; 
            cin.clear(); 
     ...
     resultado = (mate + fisica + quimica + programacion) / 4;
     return resultado;
...
alumno1.Promedio_programador(); //llamo a la funcion para obtener el promedio del alumno. este no necesita parametros ya que se obtienen por medio del usuario.
